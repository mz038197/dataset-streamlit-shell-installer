...
Agent 多半是可選裝飾。建議問句已全站廢止；卡住時走訓練前預測旁的「Agent 提示」。
...
- **page 14–18 與 K-近鄰分類**（單／多變量回歸、邏輯迴歸、正則化邏輯迴歸、線性支持向量機、K-近鄰分類）：**僅內建範例資料**，不提供 `ready.csv` 切換（2026-07-21／2026-07-22 定案；對齊決策樹／XGBoost 教學頁）。
...
- 其餘分類頁（邏輯迴歸／樹／XGBoost）全面加預測關卡或寫回（**SVM／KNN 已做雙階段＋預測**）。
- NN 級實驗迴圈套到回歸／SVM／KNN 頁。
- 硬鎖「一定要跟 Agent 講過才能訓練」（目前是答題解鎖；Agent 是輔助）。
- Kernel／非線性 SVM；Tab1 範例寫 `C=1e6`。
- KNN：`weights`／`metric` 進階項；全專案改 Plotly；confusion matrix 當主視覺。
- 無上限搜參、背景長時間訓練。
...
2. 抽共用 `pretrain_quiz` UI 殼（回歸＋SVM＋KNN 已落地，可評估是否抽殼）。
