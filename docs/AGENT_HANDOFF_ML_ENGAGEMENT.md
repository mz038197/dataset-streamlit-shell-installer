...
- 換 **feature／target** → 重置兩題；改 α／epochs **不**強制重置。
...
1. **斜率方向**（正的／負的／接近 0）  
   正解：當前資料 `feature` 與 `target` 相關係數符號（`|corr|≤0.05` → 接近 0），相容內建與 `ready.csv`。
2. **α 明顯偏大時 Cost**  
   正解固定：劇烈震盪或往上爆。
...
- 內建資料：階段1 `svm_blobs_80.csv`（可分）；階段2 `svm_soft_margin_80.csv`（重疊）。
