...
**Consequences**：ADR-0011 的「空殼含整頁工作流」改由頁骨架承擔；空殼還原來源變成兩個空函式。load／進頁抄回／清除回起點畫鈕共用挑戰軌道模組的區函式形狀判斷。Agent 預設不改頁骨架與空殼還原來源。頁骨架不在模組頂層 import live UI，並包區函式例外，避免語法錯誤或訓練失敗讓資料 Agent 欄畫不出來。Challenge host 與空殼 docstring 放可貼的 `from dataset_streamlit_shell.ml... import ...`，避免 Agent 猜錯 import。
