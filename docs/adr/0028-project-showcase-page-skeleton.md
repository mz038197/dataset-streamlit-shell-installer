...
**Consequences**：ADR-0011 的「空殼含整頁工作流」改由頁骨架承擔；空殼還原來源是兩個區函式（後補可抄的最小選型／訓練骨架，發佈時與 live 位元組相同）。load／進頁抄回／清除回起點畫鈕共用挑戰軌道模組的區函式形狀判斷。Agent 預設不改頁骨架與空殼還原來源。頁骨架不在模組頂層 import live UI，並包區函式例外，避免語法錯誤或訓練失敗讓資料 Agent 欄畫不出來。
