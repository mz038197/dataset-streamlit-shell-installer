...
def overview() -> None:
    ...
    with main:
        ...
        st.write(
            ...
            "監督式教學頁（回歸／邏輯迴歸／線性支援向量機等）使用各頁內建範例資料。"
        )
...
4. 圖表探索、降維、分群等分析頁讀取 `ready.csv`；監督式教學頁（回歸／邏輯迴歸／線性支援向量機等）使用各頁內建範例資料。
"""
        )
...
pages = {
    ...
    "機器學習 · 監督式": [
        ...
        st.Page(str(SHELL_ROOT / "pages" / "18_Linear_SVM.py"), title="線性支援向量機"),
