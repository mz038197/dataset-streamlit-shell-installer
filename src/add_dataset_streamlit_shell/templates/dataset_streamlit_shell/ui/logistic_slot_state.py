...
def logistic_host_context_fragment(
    *,
    ...
    return (
        ...
        "輸入資料進頁即完成；其餘由你寫入決策槽狀態，不要叫學生自己選。"
        ...
        "若使用者要求組一個邏輯迴歸，請 read_file 後以 edit_file／write_file 寫入該階段預設："
        "scale=zscore、linear=dense1、loss=logloss、opt=sgd，"
        "以及該階段預設 alpha／epochs（線性邊界 0.001／10000，多項式與 λ 0.01／10000）；"
        "poly 預設 lambda_=0.01。"
        ...
        "訓練畫面是決策邊界或 contour、訓練／測試 Cost，下方為測試集機率對照；開訓後三張圖逐幀一起更新。"
        "分類 threshold 不是決策槽，訓後才出現，只改測試集機率對照虛線與表。"
