...
def lr_host_context_fragment(
    *,
    ...
    return (
        ...
        "尚未被要求的 split／scale／linear／loss／opt 與 alpha、epochs 為尚未選擇。"
        "只說組模型：列出仍尚未選擇的格與各格可寫入的值，當輪不寫入。"
        "對學生用課堂名稱：線性層 Dense(1, linear)、損失 MSE、優化器 SGD、特徵縮放建議 Z分數正規化；"
        "寫入 JSON 才用 linear=dense1、loss=mse、opt=sgd、scale=zscore；"
        "優化器課堂建議 alpha／epochs 單變量 0.01／1500，多變量 0.1／1000。"
