...
- 課堂 Agent 填模型區時，host 必須給可直接貼的 `from dataset_streamlit_shell.ml.regression import ...` 整行。只寫「用 dataset_streamlit_shell.ml.regression 的 gradient_descent_steps」或「只能 import 這兩個 ml 模組」，它會寫 `from ml.xxx`、拿掉 pandas／streamlit／ChallengePaths，然後卡在 ImportError／NameError。空殼 docstring 同步放同一組 from 行；函式本體仍保持空。
- Challenge host 裡的檔案路徑必須跟 CSV 一樣走 `_display_path`（相對**專案根**，例如 `dataset_streamlit_shell/ui/startup_challenge_ui.py`）。只寫 `ui/startup_challenge_ui.py`，Agent 會在專案根建一份假的 `ui/`，頁面載入的 live 檔不會變。
- Streamlit `st.dataframe` 走 pyarrow。`df.melt()` 或 `st.line_chart`／`st.bar_chart` 吃到同時有文字欄與數字欄的寬表時，會做出 object 的 `value` 欄，混著 str 與 `numpy.int64`，然後丟 `ArrowTypeError: Expected bytes, got a 'numpy.int64' object`。畫 Cost 用兩欄都是數字的表。object 欄先轉字串再 `st.dataframe`。專案展示資料預覽走 `prepare_dataframe_for_display`。
- Challenge host 是 f-string。要在文案裡寫 `{"iteration": ...}` 這種 dict 字面值，大括號必須加倍成 `{{` `}}`，否則執行期 `ValueError: Format specifier missing precision`，host 整段組不出來。
